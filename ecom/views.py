...
def my_profile_view(request):
    try:
        ...
        # Handle case where customer doesn't exist for this user
        # You could redirect to a different view, show an error message, etc.
        # Example:
        return render(request, 'error.html', {'message': 'Customer profile not found.'})
...
def edit_profile_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            user.set_password(user.password)
            user.save()
            customerForm.save(commit=False)
             # Handle profile picture upload
            if 'profile_pic' in request.FILES:
                profile_pic = request.FILES['profile_pic']
                # Ensure the directory exists
                directory = os.path.join(settings.BASE_DIR, 'static/profile_pic/CustomerProfilePic/')
                os.makedirs(directory, exist_ok=True)
                file_path = os.path.join(directory, profile_pic.name)
                # Save the image
                with open(file_path, 'wb+') as destination:
                    for chunk in profile_pic.chunks():
                        destination.write(chunk)
                # Update the customer with the path to the image
                customer.profile_pic = os.path.join('profile_pic/CustomerProfilePic/', profile_pic.name)
